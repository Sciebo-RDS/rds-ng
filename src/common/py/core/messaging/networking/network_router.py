...
class NetworkRouter:
    ...
    def check_local_routing(
        self, direction: Direction, msg: Message, msg_meta: MessageMetaInformation
    ) -> bool:
        """
    ...
    def check_client_routing(
        ...
        if self._has_client:
            ...
            if direction == NetworkRouter.Direction.IN:
                if msg.target.is_direct and msg.target.target_id.equals(
                    self._comp_id
                ):  # Skip messages targeted to us
                    return False
    ...
    def _verify_local_message(self, direction: Direction, msg: Message) -> None:
        ...
        raise NetworkRouter.RoutingError(
            "A local message was directed to the network engine"
        )
    ...
    def _verify_direct_message(self, direction: Direction, msg: Message) -> None:
        ...
        if direction == NetworkRouter.Direction.OUT:
            if msg.target.target_id.equals(self._comp_id):
                raise NetworkRouter.RoutingError(
                    "Direct message to this component sent through the network engine"
                )
