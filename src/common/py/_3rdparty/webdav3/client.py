...
class Client(object):
    ...
    def get_headers(self, action, headers_ext=None):
        ...
        if self.webdav.token:
            webdav_token = "Authorization: Bearer {token}".format(
    ...
    def execute_request(self, action, path, data=None, headers_ext=None):
        ...
        response = self.session.request(
            ...
            auth=(
                (self.webdav.login, self.webdav.password)
            ...
            cert=(
                (self.webdav.cert_path, self.webdav.key_path)
                if (self.webdav.cert_path and self.webdav.key_path)
                else None
            ),
            data=data,
            stream=True,
    ...
    def copy(self, remote_path_from, remote_path_to, depth=1):
        ...
        headers = ["Destination: {url}".format(url=self.get_url(urn_to.quote()))]
