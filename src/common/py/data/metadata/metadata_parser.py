...
class MetadataParser:
    ...
    def validate_profile(profile: PropertyProfile) -> bool:
        ...
        Args:
            profile (PropertyProfile): The profile dictionary to validate. It should contain 'metadata' and optionally 'layout' and 'classes'.
        Returns:
        ...
        try:
            ...
            if (layout := getattr(profile, "layout")) is not None:
                import itertools
    ...
    @staticmethod
    def getattr(
        metadata: MetadataSet, query: MetadataParserQuery
    ) -> Any:
        """
    ...
    def get_profile_layout(
        ...
        Args:
            profile_metadata (PropertyProfile): A list of dictionaries containing profile metadata.

        Returns:
            ProfileLayout: A list of dictionaries representing the layout information.
        """
        
        return getattr(profile, "layout")
    ...
    def is_property_valid(
        ...
        if (
            property_layout := next(e for e in MetadataParser.get_profile_layout(profile) if e.id == prop_id)
        ) is None:
            print(f"Property {prop_id} not defined in profile")
            return False

        if not property_layout.required:
            return True
        ...
        if len(property_layout.input) > 0:
    ...
    @staticmethod
    def _transform_simple_values(obj, profile: PropertyProfile) -> List[Dict[str, str]]:
        values = []

        if (
            obj_profile := next(e for e in profile.layout if e.id == obj.id)
        ) is None or not obj.value:
            return values
        ...
        for key, value in obj.value.items():
            ...
            if not value_definition or value_definition.label == '':
                continue
    ...
    @staticmethod
    def _transform_complex_values(obj, shared_objects, profile: PropertyProfile) -> List[Dict[str, str]]:
        values = []
        ...
        def _replace_template(obj, template, profile):
            ...
            result = template
            matches = re.findall(r"\${[a-zA-z0-9]*}", result)

            for match in [m for m in matches if m is not None]:
                fallback_str = f"{{{next(e for e in profile.classes.get(obj.type).input if e.id == match[2:-1]).label}}}"
    ...
    def get_value_list(
        ...
        Args:
            ...
            prop_id (str): The property ID to look up in the metadata.
            shared_objects (list, optional): A list of shared objects that may be referenced in the metadata. Defaults to an empty list.
            profile (PropertyProfile): A dictionary containing profile information, including class layouts and templates.
