...
def parse_creators(
    ...
    for creator_id in creators_raw.refs:
        ...
        creator["affiliation"] = "; ".join(
            [
                a.value.get("affiliation", "")
                for a in creator_objects
                if a.type == "affiliation"
            ]
        )
        creator["orcid"] = "; ".join(
            [
                a.value.get("nameIdentifier", "")
                for a in creator_objects
                if a.type == "nameIdentifier"
                and a.value.get("nameIdentifierScheme", "").lower() == "orcid"
            ]
        )
        creator["gnd"] = "; ".join(
            [
                ...
                for a in creator_objects
                if a.type == "nameIdentifier"
                and a.value.get("nameIdentifierScheme", "").lower() == "gnd"
            ]
        )
...
def parse_contributors(
    contributors_raw: PropertyObject, shared_objects: List[PropertyObject]
) -> List[Dict[str, Any]]:
    """
    ...
    for contributor_id in contributors_raw.refs:
        ...
        if contributor_raw.value.get("nameType", "") == "Personal":
            ...
            contributor = {}
            contributor_objects = [
                e for e in shared_objects if e.id in contributor_raw.refs
            ]

            contributor["name"] = contributor_raw.value.get("contributorName", "")
            contributor["type"] = contributor_raw.value.get("contributorType", "")
            contributor["affiliation"] = "; ".join(
                [
                    a.value.get("affiliation", "")
                    for a in contributor_objects
                    if a.type == "affiliation"
                ]
            )
            contributor["orcid"] = "; ".join(
                [
                    a.value.get("nameIdentifier", "")
                    for a in contributor_objects
                    if a.type == "nameIdentifier"
...
def parse_subjects(
    subjects_raw: List[PropertyObject], shared_objects: List[PropertyObject]
) -> List[Dict[str, Any]]:
    pass
...
def parse_grants(
    grants_raw: PropertyObject, shared_objects: List[PropertyObject]
) -> List[Dict[str, Any]]:
    """
    ...
    for grant_id in grants_raw.refs:
        ...
        if (
            funder_id := grant_raw.value.get("funderIdentifier", "")
        ) not in funder_ids or not grant_raw.value.get("awardNumber", ""):
            continue
...
def parse_dates(
    dates_raw: PropertyObject, shared_objects: List[PropertyObject]
) -> List[Dict[str, Any]]:
    """
    ...
    for date_id in dates_raw.refs:
        ...
        date = {
            "start": date_raw.value.get("date", "")[:10],
            "end": date_raw.value.get("date", "")[:10],
            "type": date_raw.value.get("dateType", ""),
            "description": date_raw.value.get("dateInformation", ""),
        }
